 ...
 * Drupal 6 to 7 conversion by iStryker
 * @Copyright 2010 KontextWork
 */
(function ($) {
  Drupal.behaviors.tagging = {
	attach: function (context, settings) {
	  // Find all the tagging widgets inside context that do not have our
      // processed class.
      $('input.tagging-widget-input', context).once('tagging-widget', function() {
        //return this.each( function(){
        // **************** Init *****************/
    	
        /*
         * Extracts the context ID - for drupal thats the VID.
         * This is extracted from the tagging-widget-XX class
         * if the input element, which actuall has the tagging-widget class.
         */
        var widget_id = get_widget_id($(this).attr('class'));
        	
        function get_widget_id(classes) { 
          id = null;
        ...
        if(widget_id === null) {
          alert( Drupal.t('cant initialize ') + "tagging-widget: " + $(this).attr('id') + Drupal.t("..did you forget the ") + "tagging-widget-$WIDGET_ID" + Drupal.t(" class?"));
          return;
        }
      
        // Our containers.
        var input_sel = '.tagging-widget-input-'+widget_id;
        var button_sel = '.tagging-button-'+widget_id;
        var wrapper_sel = '.tagging-curtags-wrapper-'+widget_id;
        var suggestions_wrapper_sel = '.tagging-suggestions-wrapper-'+widget_id;
        var target_sel = '.tagging-widget-target-'+widget_id;
